...
/// 短视频录制类
/// @{
LITEAV_EXPORT @interface TXUGCRecord : NSObject
...
/** 设置横竖屏录制，设置后可能会改变视频预览的方向,请调用setRenderRotation
 * 来修本地视预览频流方向，请参考上面注释或则demo示例
...
/**
  ...
  每一次暂停录制都会生成一个视频片段(>100ms有效，<= 100ms
  会被认为是无效视频，不会被存放在视频分片里面)，您可以在partsManager里面管理这些视频片段
  @return  0 成功, -1 不存在录制任务, -2 videoRecorder未初始化
 */
- (int)pauseRecord;

/**
  4.9 版本后pauseRecord 修改为了异步调用，请在收到 complete 回调后再去 partsManager
  获取当前的视频片段信息 注意：resumeRecord 不需要等到 pauseRecord 的 complete
  之后再调用，正常顺序调用即可
  @return 0 成功, -1 不存在录制任务, -2 videoRecorder未初始化
 */
 ...
 * @return 0 成功, -1 不存在录制任务, -2 videoRecorder未初始化
 * @warning 生成最终视频后，当您不再使用录制过程中的生成的视频分片，请主动调用TXUGCPartsManager ->
 * deleteAllParts
 * 删除，否则视频分片会一直存在本地，如果程序没有重启，下次录制SDK也会默认加载当前视频分片
 */
- (int)stopRecord;
...
/**
  ...
  @param normalizationFrame   水印相对于视频图像的归一化frame，x,y,width,height 取值范围 0~1；
                      ...
                      比如视频图像大小为（540，960）
  frame设置为（0.1，0.1，0.1,0）,水印的实际像素坐标为（540 * 0.1，960 * 0.1，540 * 0.1 ，540 * 0.1 *
 ...
 * 注意：录制过程不能切换背景音乐，可能会导致异常，如果需要切换音乐，请先停止视频录制
 * @param asset
 * 音乐属性asset,从系统媒体库loading出来的音乐，可以直接传入对应的音乐属性，会极大的降低音乐从系统媒体库loading的时间，具体请参考demo用法
 * return: 音乐时长 s (返回值为0，表示BGM格式不支持或则音频解析失败)
 */
 ...
 *  注意：变声目前仅支持 AUDIO_SAMPLERATE_8000 , AUDIO_SAMPLERATE_16000 ,AUDIO_SAMPLERATE_48000
 * 三种采样率
 */
