...
/// 视频信息获取类
/// @{
LITEAV_EXPORT @interface TXVideoInfoReader : NSObject
/**
 ...
 */
typedef BOOL (^sampleProcess)(int number, UIImage *_Nullable image);

/** 获取视频文件信息
 * @param videoPath 视频文件路径
 */
+ (nullable TXVideoInfo *)getVideoInfo:(NSString *)videoPath;

/** 获取视频文件信息
 * @param videoAsset 视频文件属性
 * @return 视频信息
 */
+ (nullable TXVideoInfo *)getVideoInfoWithAsset:(AVAsset *)videoAsset;
...
/// 视频编辑类
LITEAV_EXPORT @interface TXVideoEditer : NSObject
...
/**
 * 设置转场图片列表 [精简版不支持]
 * @param pitureList 转场图片列表,至少设置三张图片 （tips
 * ：图片最好压缩到720P以下（参考demo用法），否则内存占用可能过大，导致编辑过程异常）
...
/**
 * 设置背景音乐 [精简版不支持]
 * @param asset      音乐AVAsset对象,
 * 从系统媒体库loading出来的音乐，可以直接传入对应的音乐属性，会极大的降低音乐从系统媒体库loading的时间，具体请参考demo用法
 * @param result     0:成功, -1:音乐文件格式不支持
 */
 ...
 *                             水印的实际像素坐标为（540 * 0.1，960 * 0.1，540 * 0.1 ，540 * 0.1 *
...
/**
  ...
  您可以调用resumeGenerate
  继续视频生成，这里需要注意的是，调用resumeGenerate，sdk会重启硬编码器，会有一定的概率重启失败，
  或则重启后前几帧数据编码失败，这个时候SDK内部会在 TXVideoGenerateListener
  抛出编码错误事件，收到错误事件后您需要重新生成视频。
...
LITEAV_EXPORT @interface TXVideoJoiner : NSObject
 ...
 */
//- (void)quickJoinVideo:(TXVideoCompressed)videoCompressed  videoOutputPath:(NSString
//*)videoOutputPath;
