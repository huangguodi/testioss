...
/// 录制参数类
LITEAV_EXPORT @interface TXUGCCustomConfig : NSObject
///自定义分辨率
@property(nonatomic, assign) TXVideoResolution videoResolution;
///自定义fps   15~30
@property(nonatomic, assign) int videoFPS;

/// 自定义码率   建议值：600~12000 (SDK上限不再做限制)
/// 单位kbps/s,这里需要注意的是，这里设置的码率只是给编码器一个参考值，实际出来视频的码率是会在这个参考值上下波动的
...
/** 是否开启回声消除（默认开启）
 @discussion
 开启回声消除，可以录制人声，BGM，人声+BGM
 （注意：录制中开启回声消除，BGM的播放模式是手机通话模式，这个模式下系统静音会失效，而视频播放预览走的是媒体播放模式，播放模式的不同会导致录制和预览在相同系统音量下播放声音大小有一定区别）
 ...
 * 录制结果
 */
LITEAV_EXPORT @interface TXUGCRecordResult : NSObject
///错误码
