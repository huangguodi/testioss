...
//        [picNameArray addObject:@"copy"];
        ...
        for (int i = 0; i < picNameArray.count; i++) {
            ...
            button.tag = i + 3000;
            NSString *imageStr = [NSString stringWithFormat:@"liveShare_%@",picNameArray[i]];
            [button setBackgroundImage:[UIImage imageNamed:getImagename(imageStr)] forState:0];
...
-(void)FenXiang:(NSDictionary *)dic{
    ...
    }else if([fenxiang isEqual:@"copy"])
    {
        ...
        if ([_zhuboDic valueForKey:@"fans"]) {
            ...
            copyStr = [copyStr stringByAppendingFormat:@"&lang=%@",[RookieTools serviceLang]];
        }else{
            copyStr = [[common wx_siteurl] stringByAppendingFormat:@"%@",[self.zhuboDic valueForKey:@"uid"]];
            if ([copyStr containsString:@"?"]) {
                copyStr = [copyStr stringByAppendingFormat:@"&lang=%@",[RookieTools serviceLang]];
            }else{
                copyStr = [copyStr stringByAppendingFormat:@"?lang=%@",[RookieTools serviceLang]];
            }
    ...
    if ([_zhuboDic valueForKey:@"shareUrl"]) {
        ...
        [shareParams SSDKSetupShareParamsByText:[common video_share_des]
                                     images:[self.zhuboDic valueForKey:@"avatar_thumb"]
                                        url:[NSURL URLWithString:[NSString stringWithFormat:@"%@/appapi/Video/share?id=%@&lang=%@",h5url,minstr([_zhuboDic valueForKey:@"videoid"]),[RookieTools serviceLang]]]
