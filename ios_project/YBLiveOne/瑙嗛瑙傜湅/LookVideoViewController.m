...
- (void)creatUI{
    ...
    if (![_model.status isEqual:@"0"]) {
        ...
        if ([minstr([_userDic valueForKey:@"id"]) isEqual:[Config getOwnID]]) {
            ...
            titleArray = @[_model.shares,_model.comments,_model.likes];
        }else{
            NSString *str = @"";
            ...
            if (str.length > 0) {
                btnArray = @[str,@"video--礼物",@"video--分享",@"video--评论",[_model.islike isEqual:@"1"] ? @"home_zan_sel":@"home_zan"];

                titleArray = @[@"",@"",_model.shares,_model.comments,_model.likes];

            }else{
                btnArray = @[@"video--礼物",@"video--分享",@"video--评论",[_model.islike isEqual:@"1"] ? @"home_zan_sel":@"home_zan"];
...
- (void)rightBtnCLick:(UIButton *)sender{
    ...
    if ([str rangeOfString:@"home_zan"].location != NSNotFound) {
        ...
        if ([str isEqual:@"video--分享"]) {
            ...
            NSString *commentStr = minstr(_model.comments);
            ...
            if (!_comment) {
                ...
                } andvideoid:_model.videoID andhostid:[Config getOwnID] count:[commentStr intValue] talkCount:^(id type) {
...
//                    YBLookVideoCell *disCell = (YBLookVideoCell*)[self.collectionView cellForItemAtIndexPath:indexPath];
                    viewsL.text =[NSString stringWithFormat:@"%@",type];
...
- (void)callBtnClick{
    if (callType == 1) {
        if (!actionView) {
            ...
            NSArray *itemArray = @[[NSString stringWithFormat:@"%@（%@%@/%@）",YZMsg(@"语音通话"),minstr([_userDic valueForKey:@"voice_value"]),[common name_coin],YZMsg(@"分钟")],[NSString stringWithFormat:@"%@（%@%@/%@）",YZMsg(@"视频通话"),minstr([_userDic valueForKey:@"video_value"]),[common name_coin],YZMsg(@"分钟")]];
            if ([YBToolClass isUp]) {
