...
- (void)startMatch{
    ...
    if ([_isauth isEqual:@"1"]) {
        url = @"Matchs.AnchorMatch";
    }else{
        url = @"Matchs.UserMatch";
    }
...
- (void)creatUI{
    ...
    if ([_isauth isEqual:@"1"]) {
        ...
        userMatchLabel.text = YZMsg(@"正在匹配主播");
...
- (void)showAlertView{
    WeakSelf;
    alert = [[YBAlertView alloc]initWithTitle:YZMsg(@"提示") andMessage:YZMsg(@"匹配尚未成功，是否继续匹配") andButtonArrays:@[YZMsg(@"退出"),YZMsg(@"继续匹配")] andButtonClick:^(int type) {
        if (type == 2) {
            [weakSelf setMatch];
        }else if (type == 1) {
            [weakSelf doReturn];
...
- (void)doCloseMatch{
    NSString *url;
    if ([_isauth isEqual:@"1"]) {
        url = @"Matchs.AnchorCancel";
    }else{
        url = @"Matchs.UserCancel";
    }
...
-(void)onRecvNewMessage:(V2TIMMessage *)msg
{
    if (msg.elemType == V2TIM_ELEM_TYPE_CUSTOM) {
        ...
        NSDictionary *jsonDic = [NSJSONSerialization JSONObjectWithData:customData options:NSJSONReadingMutableContainers error:nil];
            ...
            if ([method isEqual:@"call"]) {
                ...
                if (action == 12) {
                    ...
                    if ([_isauth isEqual:@"1"]) {
                        ...
                        userMatchLabel.text = YZMsg(@"已匹配到主播");
                        if ([_type isEqual:@"1"]) {
                            vc.liveType = @"2";
                        }else{
                            vc.liveType = @"4";
