...
- (void)requestData{
    [YBToolClass postNetworkWithUrl:@"Matchs.GetMatch" andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            ...
            [msgLabel setAttributedText:[self setattstrWithString:[NSString stringWithFormat:@"%@%@/%@（%@：%@%@/%@）",minstr([infoDic valueForKey:@"video"]),[common name_coin],YZMsg(@"分钟"),YZMsg(@"VIP用户"),minstr([infoDic valueForKey:@"video_vip"]),[common name_coin],YZMsg(@"分钟")]]];
...
- (void)creatUI{
    ...
    NSArray *array = @[YZMsg(@"视频"),YZMsg(@"语音")];
...
- (void)topBtnClick:(UIButton *)sender{
    ...
    if ([sender.titleLabel.text isEqual:YZMsg(@"视频")]) {
        [msgLabel setAttributedText:[self setattstrWithString:[NSString stringWithFormat:@"%@%@/%@（%@：%@%@/%@）",minstr([infoDic valueForKey:@"video"]),[common name_coin],YZMsg(@"分钟"),YZMsg(@"VIP用户"),minstr([infoDic valueForKey:@"video_vip"]),[common name_coin],YZMsg(@"分钟")]]];
        type = @"1";
    }else{
        [msgLabel setAttributedText:[self setattstrWithString:[NSString stringWithFormat:@"%@%@/%@（%@：%@%@/%@）",minstr([infoDic valueForKey:@"voice"]),[common name_coin],YZMsg(@"分钟"),YZMsg(@"VIP用户"),minstr([infoDic valueForKey:@"voice_vip"]),[common name_coin],YZMsg(@"分钟")]]];
        type = @"2";
