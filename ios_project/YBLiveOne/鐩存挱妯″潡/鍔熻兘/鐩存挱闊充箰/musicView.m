    ...
    if (music == 0) {
        ...
        for (NSString *str in arrayy) {
            ...
            if (isMp3) {
                ...
                if ([name containsString:@"yblive"]) {
                    name = [name stringByReplacingOccurrencesOfString:@"_yblive" withString:@""];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *docDir = [paths objectAtIndex:0];
    NSString *loadPath = [docDir stringByAppendingFormat:@"/%@*%@*%@_yblive.mp3",songid,songname,artistname];
    loadPath = [loadPath stringByReplacingOccurrencesOfString:@" " withString:@""];
    ...
    if ([manager fileExistsAtPath:loadPath]) {
        ...
        NSString *url = [NSString stringWithFormat:@"Zlivemusic.getDownurl&audio_id=%@",[dic valueForKey:@"audio_id"]];
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    [self.tableView reloadData];
    music = 1;
    NSString *url = [NSString stringWithFormat:@"Zlivemusic.searchMusic&key=%@",self.search.text];
    [YBToolClass postNetworkWithUrl:url andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            self.allArray = info;
            [self.tableView reloadData];
