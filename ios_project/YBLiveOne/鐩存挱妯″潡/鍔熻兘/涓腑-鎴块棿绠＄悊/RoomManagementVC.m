    ...
    NSArray *arr = @[YZMsg(@"我的直播间"),YZMsg(@"我的房间")];
        for (int i = 0; i < arr.count; i++) {
            ...
            if (![lagType isEqual:ZH_CN]) {
                btn.frame = CGRectMake(_window_width/2-180+i*180, 24+statusbarHeight, 180, 40);
            }
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self navi];
    leftArray = @[YZMsg(@"管理员"),YZMsg(@"禁言用户"),YZMsg(@"拉黑用户")];
...
- (void)requestData{
    [YBToolClass postNetworkWithUrl:@"Zlivemanage.getRoomList" andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            rightArray = info;
            if (rightArray.count < 1) {
                [PublicView showImgNoData:rightTable name:@"" text:YZMsg(@"你还不是管理员")];
            }else{
                [PublicView hiddenImgNoData:rightTable];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == leftTable) {
        ...
        cell.titleL.text = [NSString stringWithFormat:YZMsg(@"%@ 的直播间"),minstr([rightArray[indexPath.row] valueForKey:@"user_nickname"])];
    }
    return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == leftTable) {
        ...
        vc.titleStr =[NSString stringWithFormat:YZMsg(@"%@ 的直播间"),minstr([userinfo valueForKey:@"user_nickname"])];
