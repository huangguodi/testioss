...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = [NSString stringWithFormat:YZMsg(@"%@列表"),_titleStr];
    self.nothingMsgL.text = [NSString stringWithFormat:YZMsg(@"没有被%@"),_titleStr];
...
- (void)requestData{
    NSString *method;
    if (_type == 0) {
       method = @"Zlivemanage.GetManageList";
    }
    if (_type == 1) {
        method = @"Zlivemanage.getShutList";
    }
    if (_type == 2) {
        method = @"Zlivemanage.getKickList";
    }
...
- (void)delateAdminUser:(NSDictionary *)userDic{
    NSString *userName = minstr([userDic valueForKey:@"user_nickname"]);
    NSString *userId = minstr([userDic valueForKey:@"id"]);
    NSString *msg;
    if (_type == 0) {
        msg = [NSString stringWithFormat:YZMsg(@"是否确定取消%@的管理员身份？"),userName];
    }
    if (_type == 1) {
...
- (void)caozuo:(NSString *)userId{
    NSString *method;
    if (_type == 0) {
        method = @"Zlivemanage.cancelManage";
    }
    if (_type == 1) {
        method = @"Zlivemanage.cancelShut";
    }
    if (_type == 2) {
        method = @"Zlivemanage.cancelKick";
    }
