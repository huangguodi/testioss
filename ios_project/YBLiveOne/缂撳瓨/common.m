...
NSString *const  im_teenager_tips = @"im_teenager_tips";
NSString *const  Ksprout_key = @"sprout_key";
NSString *const  Ksprout_appid_ios = @"sprout_appid_ios";
NSString *const  yb_leaderboard_switch =  @"leaderboard_switch";
NSString *const  yb_teenager_des =  @"teenager_des";

@implementation common
+ (void)saveProfile:(liveCommon *)user andInfo:(NSDictionary *)infoDic; {
    ...
    [userDefaults setObject:user.im_teenager_tips forKey:im_teenager_tips];

    [userDefaults setObject:user.sprout_key forKey:Ksprout_key];
    [userDefaults setObject:user.sprout_appid_ios forKey:Ksprout_appid_ios];
    [userDefaults setObject:user.leaderboard_switch forKey:yb_leaderboard_switch];
    [userDefaults setObject:user.teenager_des forKey:yb_teenager_des];
    
    // 直播分类
    [common saveLiveClass:[infoDic valueForKey:@"liveclass"]];
    // 直播间类型
    [common saveLiveType:[infoDic valueForKey:@"live_type"]];
    // 计时房间收费阶层
    [common saveLiveTimeCoin:[infoDic valueForKey:@"live_time_coin"]];
    // 直播分享标题、话术
    [common saveShareLiveTitle:[infoDic valueForKey:@"share_live_title"]];
    [common saveShareLiveDes:[infoDic valueForKey:@"share_live_des"]];
    // 缺少私信开关 默认打开
    //[common saveLetter_switch:minstr([infoDic valueForKey:@"letter_switch"])];
    [common saveLetter_switch:@"1"];
    
    // IM群组
    [common save_full_group_id:minstr([infoDic valueForKey:@"full_group_id"])];
    
    [userDefaults synchronize];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"HOMECONFIGDATA" object:nil];
...
+ (void)clearProfile{
    ...
    [userDefaults setObject:nil forKey:im_teenager_tips];
    [userDefaults setObject:nil forKey:Ksprout_key];
    [userDefaults setObject:nil forKey:Ksprout_appid_ios];
    [userDefaults setObject:nil forKey:yb_leaderboard_switch];
    [userDefaults setObject:nil forKey:yb_teenager_des];
...
+ (liveCommon *)myProfile{
    ...
    user.im_teenager_tips = [userDefaults objectForKey:im_teenager_tips];
    user.sprout_key = [userDefaults objectForKey:Ksprout_key];
    user.sprout_appid_ios = [userDefaults objectForKey:Ksprout_appid_ios];
    user.leaderboard_switch = [userDefaults objectForKey:yb_leaderboard_switch];
    user.teenager_des = [userDefaults objectForKey:yb_teenager_des];
...
+(NSString *)get_im_teenager_tips{
    NSUserDefaults *userDefaults = [[NSUserDefaults alloc]init ];
    NSString* tip = [userDefaults objectForKey: im_teenager_tips];
    return tip;
...
+(NSString *)getMHAppid{
    NSUserDefaults *userDefaults = [[NSUserDefaults alloc]init ];
    NSString* key = [userDefaults objectForKey: Ksprout_appid_ios];
    return key;
}
+(NSString *)getLeaderboard_switch{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getStr = [userDefaults objectForKey:yb_leaderboard_switch];
    return getStr;
}
+(NSString *)getTeenager_des{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getStr = [userDefaults objectForKey:yb_teenager_des];
    return getStr;
}


#pragma mark - 直播分类
+(void)saveLiveClass:(NSArray *)save; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_live_class"];
    [userDefaults synchronize];
}
+(NSArray *)getLiveClass; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSArray *liveClass = [userDefaults objectForKey:@"yb_live_class"];
    return liveClass;
}
#pragma mark - 房间类型
+(void)saveLiveType:(NSArray *)save; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_live_type"];
    [userDefaults synchronize];
}
+(NSArray *)getLiveType; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSArray *getVals = [userDefaults objectForKey:@"yb_live_type"];
    return getVals;
}
#pragma mark - 计时房间收费阶级
+(void)saveLiveTimeCoin:(NSArray *)save {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_live_time_coin"];
    [userDefaults synchronize];
}
+(NSArray *)getLiveTimeCoin; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSArray *getVals = [userDefaults objectForKey:@"yb_live_time_coin"];
    return getVals;
}

#pragma mark - 直播分享标题、话术
+(void)saveShareLiveTitle:(NSString *)save {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_share_live_title"];
    [userDefaults synchronize];
}
+(NSString *)getShareLiveTitle;{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getVals = [userDefaults objectForKey:@"yb_share_live_title"];
    return getVals;
}
+(void)saveShareLiveDes:(NSString *)save {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_share_live_des"];
    [userDefaults synchronize];
}
+(NSString *)getShareLiveDes;{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getVals = [userDefaults objectForKey:@"yb_share_live_des"];
    return getVals;
}
#pragma mark - 私信开关
+(void)saveLetter_switch:(NSString *)save {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_live_letter_switch"];
    [userDefaults synchronize];
}
+(NSString *)letter_switch; {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getVals = [userDefaults objectForKey:@"yb_live_letter_switch"];
    return getVals;
}

#pragma mark - im群组
+(void)save_full_group_id:(NSString *)save {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:save forKey:@"yb_im_full_group_id"];
    [userDefaults synchronize];
}
+(NSString *)get_full_group_id {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *getVals = [userDefaults objectForKey:@"yb_im_full_group_id"];
    return getVals;
}
