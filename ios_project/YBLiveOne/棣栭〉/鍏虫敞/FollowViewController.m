...
-(void)createCollectionView{
    ...
    _collectionView.backgroundColor =RGB_COLOR(@"#f5f5f5", 1);// [UIColor whiteColor];
    self.view.backgroundColor =RGB_COLOR(@"#f5f5f5", 1);// [UIColor whiteColor];
...
-(void)pullInternet{
    ...
    [YBToolClass postNetworkWithUrl:@"Home.GetAttention" andParameter:@{@"p":@(page)} success:^(int code,id info,NSString *msg) {
        ...
        if (_infoArray.count == 0) {
            ...
            self.nothingMsgL.text =YZMsg(@"请检查网络链接后重试");
            self.nothingTitleL.text = YZMsg(@"网络请求失败");
            self.nothingBtn.hidden = NO;
            [self.nothingBtn setTitle:YZMsg(@"重试") forState:0];
        }
