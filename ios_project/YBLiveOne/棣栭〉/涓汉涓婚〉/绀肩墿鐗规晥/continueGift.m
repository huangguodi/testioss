...
-(void)GiftPopView:(NSDictionary *)giftData andLianSong:(NSString *)liansong
{
    ...
    NSString *nicename  = [giftData valueForKey:@"user_nickname"];//nickname
    NSString *giftname  = minstr([giftData valueForKey:@"giftname"]);
    
    if (![lagType isEqual:ZH_CN] && ![YBToolClass checkNull:[giftData valueForKey:@"giftname_en"]]) {
...
//    NSString *giftcount = @"1";
    ...
    NSString *sendStr = YZMsg(@"送");
    NSString *content   = [NSString stringWithFormat:@"%@ %@",sendStr,giftname];
    ...
    NSRange redRange = NSMakeRange(sendStr.length+1, giftname.length);
...
-(void)removeGiftPop:(UIView *)viewa
{
    ...
    viewa=nil;
    NSLog(@"礼物送完了");
    if ([self.delegate respondsToSelector:@selector(sendGiftEnd)]) {
        [self.delegate sendGiftEnd];
    }
