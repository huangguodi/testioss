...
-(void)reloadColl{
    [YBToolClass postNetworkWithUrl:@"Gift.GetGiftList" andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            NSDictionary *infoDic = [info firstObject];
            self.allArray = [infoDic valueForKey:@"list"];
            for (int i=0; i<self.allArray.count; i++) {
            ...
            NSString *coin = minstr([infoDic valueForKey:@"coin"]);
            user.coin = coin;
...
//        [self.collectionView registerClass:[liwucell class] forCellWithReuseIdentifier:celll];
        ...
        pageControl.pageIndicatorTintColor = RGB_COLOR(@"#dcdcdc", 1);
...
//        _buttomView.backgroundColor = RGB_COLOR(@"#323232", 0.9);
        ...
        [haohuaBtn setTitle:YZMsg(@"赠送") forState:UIControlStateNormal];
        [haohuaBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        [haohuaBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [haohuaBtn setBackgroundImage:[YBToolClass getImgWithColor:normalColors] forState:UIControlStateSelected];
        [haohuaBtn setBackgroundImage:[YBToolClass getImgWithColor:RGB_COLOR(@"#969696", 1)] forState:UIControlStateNormal];
        ...
        UIImageView *coinIconImg = [[UIImageView alloc]initWithFrame:CGRectMake(10, 13, 14, 14)];
        coinIconImg.image = [UIImage imageNamed:@"coin_Icon"];
        [_buttomView addSubview:coinIconImg];
        
        //充值lable
        _chongzhi = [[UILabel alloc] init];
        LiveUser *user = [Config myProfile];
        _chongzhi.textColor = color32;
        _chongzhi.font = [UIFont boldSystemFontOfSize:15];
...
//        _push.backgroundColor = normalColors;
        ...
        sendCount = minstr(giftCountBtn.titleLabel.text);
    }
    /*******发送礼物开始 **********/
    
    if(_isLiveRoom){
        [self liveRoomSendGift:sendCount];
    }else {
        [self originSendGift:sendCount];
    }
    
}
//直播间送礼物
-(void)liveRoomSendGift:(NSString *)sendCount {
    ...
    [YBToolClass postNetworkWithUrl:@"Zlive.sendGift" andParameter:postDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if(code == 0){
            NSDictionary *infoDic = [info firstObject];
            NSString *coin = minstr([infoDic valueForKey:@"coin"]);
            NSString *level = minstr([infoDic valueForKey:@"level"]);
            ...
            NSDictionary *socketDic = @{
                @"gifttoken":minstr([infoDic valueForKey:@"gifttoken"]),
                @"level":level,
                @"evensend":_selectModel.type
            };
            // 发送socekt
            [[YBLiveSocket shareInstance]socketSendSendGift:socketDic];
...
-(void)originSendGift:(NSString *)sendCount {
    NSDictionary *giftDic = @{
                              @"liveuid":minstr([self.pldic valueForKey:@"uid"]),
                              @"giftid":_selectModel.ID,
                              @"nums":sendCount,
                              @"showid":minstr([self.pldic valueForKey:@"showid"])
                              };
    [YBToolClass postNetworkWithUrl:@"Gift.SendGift" andParameter:giftDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            NSDictionary *info2 = [info firstObject];
            ...
            NSMutableDictionary *dic = @{
                                  @"giftname":_selectModel.giftname,
                                  @"gifticon":_selectModel.imagePath,
                                  @"giftcount":sendCount,
                                  @"isGift":@"1",
                                  @"giftid":_selectModel.ID,
                                  @"type":_selectModel.type,
                                  @"avatar":[Config getavatar],
                                  @"user_nickname":[Config getOwnNicename]
                                  }.mutableCopy;
            [dic addEntriesFromDictionary:info2];
            if([self.giftDelegate respondsToSelector:@selector(sendGiftSuccess:)]){
                [self.giftDelegate sendGiftSuccess:dic];
            }
        }else{
//            _continuBTN.hidden = YES;
...
-(void)chongzhiV:(NSString *)coins{
    if (_chongzhi) {
    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",coins]];
    NSTextAttachment *attch = [[NSTextAttachment alloc] init];
    attch.image = [UIImage imageNamed:@"right"];
