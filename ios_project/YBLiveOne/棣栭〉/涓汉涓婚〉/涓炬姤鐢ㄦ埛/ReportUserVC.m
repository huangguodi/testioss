...
@interface ReportUserVC ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate>
{
...
- (void)requetData{
    [YBToolClass postNetworkWithUrl:@"User.getReportClass" andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (!headerLabel) {
        ...
        headerLabel.text = [NSString stringWithFormat:@"   %@",YZMsg(@"选择举报的理由")];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (!tableFooterView) {
        tableFooterView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _window_width, 80+110)];
        tableFooterView.backgroundColor = bg_corlor;//_table.backgroundColor;
        ...
        [tableFooterView addSubview:jubaoTextView];
...
- (void)dojubao{
    ...
    [YBToolClass postNetworkWithUrl:@"User.setReport" andParameter:parameterDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
