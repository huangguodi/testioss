...
@interface FollowUserViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,SearchCellDelegate>{
    int page;
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = YZMsg(@"关注");
    self.nothingImgV.image = [UIImage imageNamed:@"follow_无数据"];
    self.nothingMsgL.text = YZMsg(@"你还没有关注的人");
...
- (void)requestData{
    [YBToolClass postNetworkWithUrl:@"User.GetFollowsList" andParameter:@{@"p":@(page)} success:^(int code,id info,NSString *msg) {
...
//    [cell.rightBtn setTitleColor:RGB_COLOR(@"#b3b3b3", 1) forState:0];
    ...
    cell.fromType = 2;
    cell.model = model;
    if ([model.isAtt isEqual:@"1"]) {
        [cell.rightBtn setImage:[UIImage imageNamed:getImagename(@"user_取消关注")] forState:0];
    }else{
        [cell.rightBtn setImage:[UIImage imageNamed:getImagename(@"user_加关注")] forState:0];
    }
    return cell;
}
- (void)cellBtnClick:(SearchModel *)model{
    [MBProgressHUD showMessage:@""];
    [YBToolClass postNetworkWithUrl:@"User.SetAttent" andParameter:@{@"touid":model.userID} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        [MBProgressHUD hideHUD];
        if (code == 0) {
            model.isAtt = minstr([[info firstObject] valueForKey:@"isattent"]);
            [_collectionView reloadData];
        }
//        [MBProgressHUD showError:msg];
    } fail:^{
