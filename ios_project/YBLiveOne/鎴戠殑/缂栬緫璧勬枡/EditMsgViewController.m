...
@interface EditMsgViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,TZImagePickerControllerDelegate>{
    UIImage *selectImage;
    UITextField *nameT;
    UIButton *photoBtn;
}
...
- (void)rightBtnClick{
    if (nameT.text == nil || nameT.text == NULL || nameT.text.length == 0) {
        [MBProgressHUD showError:YZMsg(@"名字不能为空")];
        return;
    }
    if (nameT.text.length > 7) {
        [MBProgressHUD showError:YZMsg(@"名字最长为7位")];
        return;
    }
