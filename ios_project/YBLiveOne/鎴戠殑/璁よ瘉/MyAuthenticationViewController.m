...
@interface MyAuthenticationViewController ()<UITableViewDelegate,UITableViewDataSource,authPicCellDelegate,TZImagePickerControllerDelegate,authTextViewCellDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UIGestureRecognizerDelegate>{
...
- (void)viewDidLoad {
    ...
    [self.rightBtn setTitle:YZMsg(@"提交") forState:0];
    self.rightBtn.userInteractionEnabled = NO;
    [self.rightBtn setTitleColor:[normalColors colorWithAlphaComponent:0.3] forState:0];
    leftArray = @[YZMsg(@"*请上传一张本人真实照片，将作为列表封面展示"),YZMsg(@"*请务必上传至少一张本人真实照片，将作为个人主页背景墙展示"),YZMsg(@"真实姓名"),YZMsg(@"手机号码"),YZMsg(@"性别"),YZMsg(@"身高"),YZMsg(@"体重"),YZMsg(@"星座"),YZMsg(@"形象标签"),YZMsg(@"所在城市"),YZMsg(@"个人介绍"),YZMsg(@"个性签名")];
    placeholdArray = @[@"",@"",YZMsg(@"请输入真实姓名(必填)"),YZMsg(@"请输入手机号码(必填)"),YZMsg(@"请选择性别(必填)"),YZMsg(@"请输入身高cm(必填)"),YZMsg(@"请输入体重kg(必填)"),YZMsg(@"请选择星座(必填)"),YZMsg(@"请选择形象标签(必填)"),YZMsg(@"请选择所在城市(必填)"),YZMsg(@"请编辑个人介绍(必填)"),YZMsg(@"请编辑个性签名(必填)")];
...
//    starShowArray = @[@"白羊座(3.21-4.19)",@"金牛座(4.20-5.20)",@"双子座(5.21-6.21)",@"巨蟹座(6.22-7.22)",@"狮子座(7.23-8.22)",@"处女座(8.23-9.22)",@"天秤座(9.23-10.23)",@"天蝎座(10.24-11.22)",@"射手座(11.23-12.21)",@"摩羯座(12.22-1.19)",@"水瓶座(1.20-2.18)",@"双鱼座(2.19-3.20)"];
    ...
    if (_subDic) {
        ...
        if ([lagType isEqual:EN] && ![YBToolClass checkNull:minstr([_subDic valueForKey:@"name"])]) {
            nameStr = minstr([_subDic valueForKey:@"name"]);
        }
        ...
        sexStr = [[Config getSex] isEqual:@"1"] ? @"1":@"2";
    }
}
- (void)creatUI{
...
//    [MBProgressHUD showError:@"已提交"];
    /*
    [MBProgressHUD showMessage:YZMsg(@"正在提交认证")];
...
-(void)startUpload {
    if (singlePicArray.count>0) {
        [MBProgressHUD showMessage:YZMsg(@"正在提交认证")];
        id image = singlePicArray[0];
        if ([image isKindOfClass:[UIImage class]]) {
            [self startUploadPics:(UIImage *)image];
        }else {
            thumbStr = [YBStorageManage yb_storageLinkConvert:(NSString *)image];
            [self startUploadPics:nil];
        }
    }
}

-(void)startUploadPics:(UIImage *)singleImg {
    ...
    if (singleImg) {
        ...
        dispatch_group_async(group, queue, ^{
            [[YBStorageManage shareManage]yb_storageImg:singleImg andName:coverImgName progress:^(CGFloat percent) {
    ...
    thumbListStr = @"";
    NSMutableArray <NSString *>*uploadFinishA = [NSMutableArray array];
    for (int i = 0; i < numPicArray.count ; i ++) {
        id image = numPicArray[i];
        if ([image isKindOfClass:[UIImage class]]) {
            NSString *imageName = [YBToolClass getNameBaseCurrentTime:[NSString stringWithFormat:@"_backimage%d_auth.png", i]];
            dispatch_group_async(group, queue, ^{
                [[YBStorageManage shareManage]yb_storageImg:image andName:imageName progress:^(CGFloat percent) {
                    ...
                    [uploadFinishA addObject:key];
                    dispatch_semaphore_signal(semaphore);
                }];
                dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
            });
        }else {
            NSString *convertThumb = [YBStorageManage yb_storageLinkConvert:(NSString *)image];
            [uploadFinishA addObject:convertThumb];
        }
    }
    WeakSelf;
    dispatch_group_notify(group, queue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [MBProgressHUD hideHUD];
            //升序处理保证 uploadFinishA 的地址和 imageArray 一一对应
            NSArray *resultA = [uploadFinishA sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
                return  [obj1 compare:obj2];
            }];
            for (NSString *str in resultA) {
                if (thumbListStr.length == 0) {
                    thumbListStr = str;
                }else{
                    thumbListStr = [NSString stringWithFormat:@"%@,%@",thumbListStr,str];
                }
            }
