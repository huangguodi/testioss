...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = YZMsg(@"提现账户");
    self.rightBtn.hidden = NO;
    [self.rightBtn setTitle:YZMsg(@"添加") forState:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (type) {
        case 1:
            cell.typeImgView.image = [UIImage imageNamed:getImagename(@"profit_alipay")];
...
- (void)delateIndex:(NSInteger)index{
    UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:YZMsg(@"温馨提示") message:YZMsg(@"是否确定删除此提现账号？") preferredStyle:UIAlertControllerStyleAlert];
