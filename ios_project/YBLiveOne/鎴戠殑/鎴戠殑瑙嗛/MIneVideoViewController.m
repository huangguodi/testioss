...
- (UICollectionView *)videoCollectionV{
    if (!_videoCollectionV) {
        ...
        _videoCollectionV.mj_footer = [MJRefreshBackFooter footerWithRefreshingBlock:^{
            videoPage ++;
            [self pullVideoList];
        }];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = YZMsg(@"我的视频");
    self.nothingMsgL.text = YZMsg(@"你还没有上传过视频");
    self.nothingMsgL.font = [UIFont systemFontOfSize:14];
    self.nothingImgV.image = [UIImage imageNamed:@"follow_无数据"];

    self.rightBtn.hidden = NO;
    if (_isSelect) {
        [self.rightBtn setTitle:YZMsg(@"确定") forState:0];
    }else{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(uploadSuccess) name:@"RELOADHOMEVIDEOLIST" object:nil];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_isSelect) {
        ...
        [[YBAppDelegate sharedAppDelegate] pushViewController:look animated:YES];
    }
}
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.status isEqual:@"0"]) {
        ...
        cell.stateL.text = [NSString stringWithFormat:@"  %@  ",YZMsg(@"审核中")];
    }else{
        cell.stateL.hidden = YES;
    }
    if ([model.isprivate isEqual:@"1"]){
        cell.priveImgV.hidden = NO;
    }else{
        cell.priveImgV.hidden = YES;
    }
    if (_isSelect) {
...
- (void)rightBtnClick{
    if (_isSelect) {
        if (selectModel) {
            ...
            [MBProgressHUD showError:YZMsg(@"请选择图片")];
        }
        ...
        UIAlertAction *photoAction = [UIAlertAction actionWithTitle:YZMsg(@"本地视频") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)showAlertView{
    WeakSelf;
    alert = [[YBAlertView alloc]initWithTitle:YZMsg(@"提示") andMessage:YZMsg(@"该视频为私密视频，设置为背景墙后将会自动转成公开视频，是否设置") andButtonArrays:@[YZMsg(@"取消"),YZMsg(@"设置")] andButtonClick:^(int type) {
        if (type == 2) {
...
//        .preferredLanguage = [lagType isEqual:ZH_CN] ? @"zh-Hans":@"en";
    ...
    [[TZImageManager manager] getVideoOutputPathWithAsset:asset presetName:AVAssetExportPresetMediumQuality success:^(NSString *outputPath) {
        ...
        [MBProgressHUD hideHUD];
        [MBProgressHUD showError:YZMsg(@"视频导出失败")];
