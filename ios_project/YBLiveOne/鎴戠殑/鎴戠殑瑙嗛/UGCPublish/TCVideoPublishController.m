...
//        [_livePlayer startLivePlay:_recordResult.videoPath type:PLAY_TYPE_LOCAL_VIDEO];
        self.videoPlayer.assetURL =[NSURL fileURLWithPath:_recordResult.videoPath];
...
//    [BGSetting getBgSettingUpdate:NO maintain:NO eventBack:nil];
    if ([YBToolClass isUp]) {
        _locationV.hidden = _tipsLable.hidden = YES;
    }
}
...
-(void)doReturn
{
    UIAlertController *returnAlert = [UIAlertController alertControllerWithTitle:YZMsg(@"提示") message:YZMsg(@"是否放弃发布此条视频") preferredStyle:UIAlertControllerStyleAlert];
    ...
    [returnAlert addAction:sureAction];
    [self.navigationController presentViewController:returnAlert animated:YES completion:nil];
}
#pragma mark - 发布
...
-(void)startUpload {
    ...
    if (_recordResult.coverImage) {
        dispatch_group_async(group, queue, ^{
            ...
            [[YBStorageManage shareManage]yb_storageImg:_recordResult.coverImage andName:imageName progress:^(CGFloat percent) {
    ...
    if (![YBToolClass checkNull:filePath]) {
        dispatch_group_async(group, queue, ^{
            NSString *videoName = [YBToolClass getNameBaseCurrentTime:@".mp4"];
    ...
    dispatch_group_notify(group, queue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (uploadStateCode == 0) {
                [weakself requstAPPServceTitle:mytitle andVideo:_videokey andImage:_imagekey];
            }else {
...
-(UIView *)locationV {
    if (!_locationV) {
        ...
        label.text = YZMsg(@"私密设置");
        [_locationV addSubview:label];
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_locationV.mas_left).offset(10);
            make.top.equalTo(_locationV);
            make.height.mas_equalTo(50);
        }];
        [_locationV layoutIfNeeded];
...
//    NSDictionary *dic;
    ...
    if (index > videoMaxSelectIndex) {
        [MBProgressHUD showError:@"等级不够,无法使用此价格"];
        return;
    }
...
-(UIButton *)publishBtn {
    if (!_publishBtn) {
        ...
        [_publishBtn setTitle:YZMsg(@"确认发布") forState:0];
...
//        UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:YZMsg(YZMsg(@"提示")) message:YZMsg(@"是否放弃发布此条视频") preferredStyle:UIAlertControllerStyleAlert];
//        UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:YZMsg(YZMsg(@"取消")) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        }];
...
-(ZFPlayerController *)videoPlayer{
    if(!_videoPlayer){
        ...
        _videoPlayer =[ZFPlayerController playerWithPlayerManager:playerManager containerView:_videoPreview];
        ...
        _videoPlayer.currentPlayerManager.scalingMode = ZFPlayerScalingModeAspectFill;
    }
    return _videoPlayer;
}
