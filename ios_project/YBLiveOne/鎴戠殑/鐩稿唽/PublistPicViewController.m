...
@interface PublistPicViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UITableViewDelegate,UITableViewDataSource,TZImagePickerControllerDelegate>{
    UIImage *selectImage;
...
- (void)messageBtnClick{
    //Video.getFeeInfo
    [YBToolClass postNetworkWithUrl:@"Photo.getFeeinfo" andParameter:nil success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto{
    ...
    [selectImgBtn setImage:selectImage forState:UIControlStateNormal];
}
...
- (void)doReturn{
    if (selectImage) {
        UIAlertController *alertContro = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
        UIAlertAction *photoAction = [UIAlertAction actionWithTitle:YZMsg(@"放弃上传") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)rightBtnClick{
    if (!selectImage) {
        [MBProgressHUD showError:YZMsg(@"请选择照片")];
        return;
    }
...
- (void)uploadimagesuccess:(NSString *)thumb{
    ...
    [YBToolClass postNetworkWithUrl:@"Photo.setPhoto" andParameter:pullDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            [MBProgressHUD hideHUD];
            [MBProgressHUD showError:YZMsg(@"上传成功，请等待审核")];
            selectImage = nil;
            [selectImgBtn setImage:[UIImage imageNamed:@"auth_正"] forState:0];
