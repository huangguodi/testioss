...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = YZMsg(@"我的相册");
    self.nothingMsgL.text = YZMsg(@"你还没有上传过照片");
...
- (void)showAlertView{
    WeakSelf;
    alert = [[YBAlertView alloc]initWithTitle:YZMsg(@"提示") andMessage:YZMsg(@"该照片为私密照片，设置为背景墙后将会自动转成公开照片，是否设置") andButtonArrays:@[YZMsg(@"取消"),YZMsg(@"设置")] andButtonClick:^(int type) {
        if (type == 2) {
