...
@interface LikeUserVC ()<UICollectionViewDelegate,UICollectionViewDataSource,SearchCellDelegate>{
    int page;
}
...
@implementation LikeUserVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleL.text = YZMsg(@"喜欢");
    self.nothingImgV.image = [UIImage imageNamed:@"follow_无数据"];
    self.nothingMsgL.text = YZMsg(@"你还没有喜欢的人");
...
- (void)requestData{
    [YBToolClass postNetworkWithUrl:@"User.getLikeList" andParameter:@{@"p":@(page)} success:^(int code,id info,NSString *msg) {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.rightBtn setTitle:[NSString stringWithFormat:@"    %@    ",YZMsg(@"私信")] forState:0];
