...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    model.isSelected = YES;
    [[NSUserDefaults standardUserDefaults] setObject:@(indexPath.row) forKey:kOneclickIndex];
    ...
    if (indexPath.row == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:@"NO" forKey:kMHSelectedQuickBeauty];
        [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:kOneclickBeauty];//清空保存的一键美颜数据
    } else {
         [[NSUserDefaults standardUserDefaults] setObject:@"YES" forKey:kMHSelectedQuickBeauty];
        if(cancelOtherBeauty){
            [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:kMHFaceTitle];
            [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:kMHBeautyTitle];
        }
