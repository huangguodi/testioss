...
- (void)configureUI {
    ...
    [self.beautyView configureBeautyData:self.beautyArr];
    [self.faceView configureFaceData:self.facesArr];
//    self.slider.maximumValue = 9;
    ...
    if(IsDictionaryWithAnyKeyValue(indexDic)){
        ...
        self.slider.hidden = index.integerValue == 0;
        self.nameLabel.hidden = index.integerValue == 0;
        NSString *beautKey = [NSString stringWithFormat:@"beauty_%ld",index.integerValue];
        NSString *value = [[NSUserDefaults standardUserDefaults] objectForKey:beautKey];
        self.nameLabel.text = YZMsg(indexDic.allKeys.firstObject);
        [self.slider setValue:value.integerValue animated:YES];
    }else{
...
- (void)switchList:(WNSegmentControl *)segment {
    ...
    self.nameLabel.hidden = [view isEqual:self.filtersView];
//    self.assembleType = segment.selectedSegmentIndex;
    ...
    if ([view isEqual:self.faceView]) {
        ...
        if(IsDictionaryWithAnyKeyValue(indexDic)){
            ...
            self.slider.hidden = index.integerValue == 0;
            self.nameLabel.hidden = index.integerValue == 0;
            NSString *beautKey = [NSString stringWithFormat:@"face_%ld",index.integerValue];
            NSString *value = [[NSUserDefaults standardUserDefaults] objectForKey:beautKey];
            self.nameLabel.text = YZMsg(indexDic.allKeys.firstObject);
            [self.slider setValue:value.integerValue animated:YES];
        }else{
...
- (void)configureOneclickDefaultData{
    ...
    if (data && ![data isKindOfClass:[NSString class]]) {
        ...
        self.nameLabel.text = YZMsg(lastModel.beautyTitle);
...
//    }
    ...
    if ([self.delegate respondsToSelector:@selector(beautyLevel:whitenessLevel:ruddinessLevel: brightnessLevel:)]) {
        if(cancelOtherBeauty){
            NSString *str = [[NSUserDefaults standardUserDefaults] objectForKey:kMHSelectedQuickBeauty];
            if ([str isEqualToString:@"YES"]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:@"NO" forKey:kMHSelectedQuickBeauty];//保证只执行一次
            }
        }
...
- (void)handleBeautyEffects:(NSInteger)type sliderValue:(NSInteger)value name:(nonnull NSString *)name{
    ...
    self.nameLabel.text = YZMsg(name);
}
//美型
...
- (void)handleCompleteEffect:(MHBeautiesModel *)model defaultSliderValue:(NSInteger)defaultValue {
    ...
    _nameLabel.text = YZMsg(model.beautyTitle);
