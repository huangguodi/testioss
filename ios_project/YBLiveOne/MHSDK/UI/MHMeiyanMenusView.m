...
@property(nonatomic,strong)NSMutableArray *facesArray;//处理美型数据
@property(nonatomic,strong)NSMutableArray *beautiesArray;//处理美颜数据
@end
@implementation MHMeiyanMenusView
#pragma mark - 调用上次保存美颜数据/第一次使用读取默认数据
- (void)setupDefaultBeautyAndFaceValue{
    /***
     针对自己项目的实际情况修改MHBeautyParams.h 文件里的kDefaultValueFromLocal。NO表示从接口获取，YES表示读取本地plist文件的数据.默认是YES
     ***/
    ...
    if (IsArrayWithAnyItem(faceArr)) {
        for (int i = 0; i<faceArr.count; i++) {
            ...
            //读取之前设置的美型数值，如果是未设置过，则点击后读取默认的数值,默认数值参考MHFaceParams.plist
...
//            //读取之前设置的数值，如果是未设置过，则点击后读取默认的数值,默认数值参考MHBeautyParams.plist
            NSString *value = [[NSUserDefaults standardUserDefaults] objectForKey:faceKey];
            if (i != 0) {
...
//            }
    ...
    [self configureBeautyData];
    [self configureFaceData];
    
}
#pragma mark - 处理美颜数据
- (void)configureBeautyData{
    if (IsArrayWithAnyItem(self.beautiesArray)) {
        return;//保证只初始化加载一次
    }
    ...
    _beautiesArray = [NSMutableArray array];
    ...
    if(IsDictionaryWithAnyKeyValue(indexDic)){
        title = indexDic.allKeys.firstObject;
    }
    BOOL hasValue = [[NSUserDefaults standardUserDefaults] boolForKey:@"HasDefaultValue_Beauty"];
    for (int i = 0; i<selectedItems.count; i++) {
        ...
        if(kDefaultValueFromLocal && !hasValue){
            NSString *beautKey = [NSString stringWithFormat:@"beauty_%ld",model.type];
            [[NSUserDefaults standardUserDefaults] setInteger:model.originalValue.integerValue forKey:beautKey];//存储默认数值
        }
        [_beautiesArray addObject:model];
    }
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"HasDefaultValue_Beauty"];
}
#pragma mark - 处理美型数据
- (void)configureFaceData {
    NSMutableArray * selectedItemArray = [MHSDK shareInstance].faceArr;
    _facesArray = [NSMutableArray arrayWithCapacity:selectedItemArray.count];
    ...
    if(IsDictionaryWithAnyKeyValue(indexDic)){
        selectTitle = indexDic.allKeys.firstObject;
    }
    BOOL hasValue = [[NSUserDefaults standardUserDefaults] boolForKey:@"HasDefaultValue_Face"];
    for (int i = 0; i<selectedItems.count; i++) {
        ...
        model.type = [itemDic[@"type"] integerValue];
        model.originalValue = itemDic[@"value"];
        model.isSelected = [model.beautyTitle isEqual:selectTitle];
        if(kDefaultValueFromLocal && !hasValue){
        ...
        [_facesArray addObject:model];
    }
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"HasDefaultValue_Face"];
}
//是否显示美颜界面
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([currentModel.beautyTitle isEqual:@""]) {
        ...
        self.beautyAssembleView.facesArr = self.facesArray;
        self.beautyAssembleView.beautyArr = self.beautiesArray;
...
-(NSMutableArray *)array {
    if (!_array) {
        NSArray * itemArray = @[@{@"贴纸":StickerImg},@{@"美颜":BeautyImg},@{@"":CameraImg},@{@"特效":SpecificImg},@{@"美妆":MakeUpImg},@{@"哈哈镜":HahaImg}];
