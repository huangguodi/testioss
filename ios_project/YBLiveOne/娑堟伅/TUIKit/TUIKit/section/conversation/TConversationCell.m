...
- (void)setData:(TConversationCellData *)data
{
    ...
    if ([_data.isVIP isEqual:@"1"] && ![YBToolClass isUp]) {
...
//    _titleLabel.text = _data.title;
    ...
    if([_data.convId isEqual:_data.liveUid]){
        _timeLabel.hidden = YES;
        _chatBtn.hidden = NO;
        [self layoutIfNeeded];
        _chatBtnWidth = CGRectGetWidth(_chatBtn.frame) + 15;
    }else{
...
- (void)setupViews
{
    ...
    _chatBtn.backgroundColor = normalColors;
    _chatBtn.userInteractionEnabled = NO;
    [_chatBtn setTitle:YZMsg(@"私聊") forState:0];
    _chatBtn.titleLabel.font = SYS_Font(13);
    [_chatBtn setTitleColor:RGB_COLOR(@"#ffffff", 1) forState:0];
    _chatBtn.contentEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 10);
    _chatBtn.layer.cornerRadius = 13;
...
- (void)defaultLayout
{
    ...
    _unReadView.frame = CGRectMake(size.width - TConversationCell_Margin - _unReadView.frame.size.width-_chatBtnWidth, size.height - TConversationCell_Margin_Text - _unReadView.frame.size.height, _unReadView.frame.size.width, _unReadView.frame.size.height);
    
    [_subTitleLabel sizeToFit];
    _subTitleLabel.frame = CGRectMake(_titleLabel.frame.origin.x, size.height - TConversationCell_Margin_Text - _subTitleLabel.frame.size.height, size.width - _headImageView.frame.size.width - 4 * TConversationCell_Margin - _unReadView.frame.size.width-_chatBtnWidth, _subTitleLabel.frame.size.height);
}
