...
- (void)downloadSnapshot:(TDownloadProgress)progress response:(TDownloadResponse)response
{
    ...
    if(isExist){
        ...
        V2TIMVideoElem *imVideo = [self getIMVideo];

        //网络下载
//        V2TIMSnapshot *imSnapshot = [self getIMSnapshot];
        ...
        [imVideo downloadSnapshot:path progress:^(NSInteger curSize, NSInteger totalSize) {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)downloadVideo:(TDownloadProgress)progress response:(TDownloadResponse)response
{
    ...
    if(isExist){
        ...
        [imVideo downloadVideo:path progress:^(NSInteger curSize, NSInteger totalSize) {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
//            V2TIMVideoElem *imVideoElem = (TIMVideoElem *)imElem;
...
- (V2TIMVideoElem *)getIMVideo
{
    V2TIMMessage *imMsg = self.custom;
    V2TIMVideoElem *imVideoElem = imMsg.videoElem;
    return imVideoElem;
//    for (int i = 0; i < imMsg.elemCount; ++i) {
//        TIMElem *imElem = [imMsg getElem:i];
//        if([imElem isKindOfClass:[TIMVideoElem class]]){
