...
@interface TMessageController () <V2TIMAdvancedMsgListener, TMessageCellDelegate, AVAudioPlayerDelegate, TAlertViewDelegate>{
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:ybImNeedRefresh object:nil];
...
-(void)onRecvNewMessage:(V2TIMMessage *)msg {
    if([msg.userID isEqual:_conv.convId]){
        NSLog(@"------收到新消息---%@",msg);
        [[YBImManager shareInstance]onRecvNewMessage:msg complete:^(TMessageCellData *receData) {
            if(receData){
                ...
                receData.userHeader = receData.isSelf ? [Config getavatar] : _conv.userHeader;;
...
//        TIMConversation * conv = [[TIMManager sharedInstance] getConversation:TIM_C2C receiver:_conv.convId];
//        [conv setReadMessage:msg succ:nil fail:nil];
...
//
//                NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:custom.data options:NSJSONReadingMutableContainers error:nil];
//                if ([dic valueForKey:@"method"]) {
//                    if ([minstr([dic valueForKey:@"method"]) isEqual:@"sendgift"]) {
//                        if (!_isLoadingMsg) {
//                            [self.delegate reciveGiftMessage:dic];
//                        }
...
//                            case 10:
////                                if (msg.isSelf) {
////                                    callData.content = @"推流成功";
////                                }else{
////                                    callData.content = @"对方推流成功";
////                                }
//                                continue;
//                                break;
//                            case 11:
...
//                    data.name = [Config getOwnNicename];
//                }else{
//                    data.userHeader = _conv.userHeader;
//                    data.name = _conv.userName;
//                }
//
//                data.custom = msg;
//            }
...
-(void)addImNum{
    [YBToolClass postNetworkWithUrl:@"Im.addImNum" andParameter:@{@"uid":[Config getOwnID],@"token":[Config getOwnToken]} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
- (void)transUIMsgFromIMMsg:(NSArray *)msgs complete:(IMMArrayBlock)imComplete{
    ...
    dispatch_group_notify(_rk_group, rk_queue, ^{
        ...
        //此处解决发送礼物由于时间戳相同排序覆盖的问题
//        for (TMessageCellData *cellData in rk_uiMsgs) {
//            [sortDic setObject:cellData forKey:cellData.timestamp];
//        }
        
        for(int i = 0 ; i < rk_uiMsgs.count; i++){
            TMessageCellData *cellData =rk_uiMsgs[i];
            NSString *kayStr = [NSString stringWithFormat:@"%@%d",cellData.timestamp,i];
            [sortDic setObject:cellData forKey:kayStr];
        }
        ...
        */
        NSArray *sortResult = [sortAllKey sortedArrayUsingComparator:^NSComparisonResult(id _Nonnull obj1, id _Nonnull obj2) {
            // 提取时间戳中的数字部分（假设格式为 "+0000X"）
            NSInteger num1 = [[obj1 componentsSeparatedByString:@"+"].lastObject integerValue];
            NSInteger num2 = [[obj2 componentsSeparatedByString:@"+"].lastObject integerValue];
            // 先比较时间部分（忽略最后数字）
            NSRange range1 = NSMakeRange(0, [obj1 rangeOfString:@"+"].location);
            NSRange range2 = NSMakeRange(0, [obj2 rangeOfString:@"+"].location);
            NSString *timePart1 = [obj1 substringWithRange:range1];
            NSString *timePart2 = [obj2 substringWithRange:range2];
            NSComparisonResult timeResult = [timePart1 compare:timePart2];
            // 如果时间部分相同，则比较数字部分
...
-(void)didTapHeaderMessage:(TMessageCellData *)data{
    if ([_conv.convId isEqual:@"admin"] || _uiFrom == Live_Im_Samll){
        return;
    }
    V2TIMMessage *imMsg = data.custom;
        if(imMsg == nil || imMsg.isSelf){
            return;
        }
    //imMsg.sender
    [MBProgressHUD showMessage:@""];
    [YBToolClass postNetworkWithUrl:@"User.GetUserHome" andParameter:@{@"liveuid":imMsg.sender} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
...
//    //[items addObject:[[UIMenuItem alloc] initWithTitle:YZMsg(@"复制") action:@selector(onDelete:)]];
//    if([data isKindOfClass:[TTextMessageCellData class]]){
//        [items addObject:[[UIMenuItem alloc] initWithTitle:YZMsg(@"复制") action:@selector(onCopy)]];
//    }
...
//        if (imMsg.isSelf) {
//            [items addObject:[[UIMenuItem alloc] initWithTitle:YZMsg(@"撤回") action:@selector(onRevoke)]];
//        }
//
//        //}
//    }
...
- (void)didReSendMessage:(TMessageCellData *)data
{
    if (data.isSelf) {
        _reSendUIMsg = data;
        TAlertView *alert = [[TAlertView alloc] initWithTitle:YZMsg(@"确定重发此消息吗？")];
        alert.delegate = self;
...
- (void)revokeMsg:(TMessageCellData *)msg
{
    ...
    if(imMsg.isSelf){
        ...
        data.content = [NSString stringWithFormat:@"\%@\%@", YZMsg(@"对方"),YZMsg(@"撤回了一条消息")];
    }
    [_uiMsgs insertObject:data atIndex:index];
    [_heightCache insertObject:[NSNumber numberWithFloat:70] atIndex:index];
...
- (void)sendCustomMessage:(NSDictionary *)dic{
    ...
    gift.giftIcon = minstr([dic valueForKey:@"gifticon"]);
    gift.giftNum = minstr([dic valueForKey:@"giftcount"]);
    gift.giftName = minstr([dic valueForKey:@"giftname"]);
    if (![lagType isEqual:ZH_CN] && ![YBToolClass checkNull:minstr([dic valueForKey:@"giftname_en"])]) {
        gift.giftName = minstr([dic valueForKey:@"giftname_en"]);
    }
...
//        imMsg = [self transIMMsgFromUIMsg:gift];
        imMsg = [[YBImManager shareInstance]transIMMsgFromUIMsg:gift];        dateMsg = [self transSystemMsgFromDate:imMsg.timestamp];
