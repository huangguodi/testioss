...
- (void)setUiFrom:(LiveEnum)uiFrom {
    _uiFrom = uiFrom;
    _textView.uiFrom = uiFrom;
}
- (void)setupViews
{
    ...
    _textView.uiFrom = _uiFrom;
    _textView.delegate = self;
    if ([[YBYoungManager shareInstance]isOpenYoung]) {
        _textView.placeholdLabel.text = [common get_im_teenager_tips];
    }else{
        _textView.placeholdLabel.text = [common im_tips];

    }
    [self.view addSubview:_textView];
}
-(void)textViewChatDidChange:(TTextView *)textView
{
    if (textView.inputTextView.text.length > 0) {
        _textView.placeholdLabel.hidden = YES;
    }else{
        _textView.placeholdLabel.hidden = NO;
    }
}

- (void)keyboardWillHide:(NSNotification *)notification
{
    if (_textView.inputTextView.text.length > 0) {
        _textView.placeholdLabel.hidden = YES;
    }else{
        _textView.placeholdLabel.hidden = NO;
    }
    if ([self.delegate respondsToSelector:@selector(inputResetHeight)]) {
        [self.delegate inputResetHeight];
    }
...
- (void)keyboardWillChangeFrame:(NSNotification *)notification
{
    ...
    ///===> 私信小窗
    if (keyboardFrame.origin.y != _window_height) {
        // 键盘弹起【注意不要在 keyboardWillShow 处理】
        CGFloat keyHeight = keyboardFrame.size.height;
        [[NSNotificationCenter defaultCenter] postNotificationName:ybImSamllChange object:@(keyHeight)];
    }else{
        // 键盘落下【注意不要在 keyboardWillHide 处理】
...
{
    ///===> 私信小窗
    [[NSNotificationCenter defaultCenter] postNotificationName:ybImSamllChange object:@(TFaceView_Height)];
    ///<=== 私信小窗
...
//    }
    ...
    NSDictionary *parDic = @{@"uid":[Config getOwnID],@"token":[Config getOwnToken],@"content":text};
    [YBToolClass postNetworkWithUrl:@"Im.checkIMMsg" andParameter:parDic success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        if (code == 0) {
            NSLog(@"----------info:-----%@",info);
            NSString *newText =minstr([[info firstObject] valueForKey:@"str"]);
