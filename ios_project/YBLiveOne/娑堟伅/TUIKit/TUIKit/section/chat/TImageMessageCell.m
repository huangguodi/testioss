...
- (NSString *)getImagePath:(TImageType)type isExist:(BOOL *)isExist {
    ...
    *isExist = NO;
    ...
    if(!*isExist){
        //查看本地是否存在
        /*
         TImageItem *tImageItem =[[TImageItem alloc] init];
         tImageItem.type =type;
         */
...
- (void)downloadImage:(TImageType)type progress:(TDownloadProgress)progress response:(TDownloadResponse)response {
    ...
    if(isExist){
        ...
        //网络下载
        // V2TIMImage *imImage = [[V2TIMImage alloc]init];//[self getIMImage:type];
...
- (V2TIMImage *)getIMImage:(TImageType)type{
    V2TIMMessage *imMsg = self.custom;
    for (int i = 0; i < imMsg.imageElem.imageList.count; ++i) {
        V2TIMImage *imElem = imMsg.imageElem.imageList[i];
        if(type == TImage_Type_Thumb && imElem.type == V2TIM_IMAGE_TYPE_THUMB){
            return imElem;
        }else if(type == TImage_Type_Origin && imElem.type == V2TIM_IMAGE_TYPE_ORIGIN){
            return imElem;
        }
        else if(type == TImage_Type_Large && imElem.type == V2TIM_IMAGE_TYPE_LARGE){
            return imElem;
        }
    }
    return nil;
...
- (void)setData:(TImageMessageCellData *)data; {
    ...
    if(data.thumbImage){
        ...
        [data downloadImage:TImage_Type_Origin progress:^(NSInteger curSize, NSInteger totalSize) {
