...
- (void)navi{
    if ([[Config getIsauth] isEqual:@"1"]) {
        ...
        NSArray *arr = @[YZMsg(@"预约我的"),YZMsg(@"我预约的")];
    ...
    [YBToolClass postNetworkWithUrl:method andParameter:@{@"p":@(page)} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
        ...
        if (code == 0) {
            ...
            if (_infoArray.count == 0) {
                ...
                if ([method isEqual:@"Subscribe.GetTome"]) {
                    [PublicView showImgNoData:_collectionView name:@"follow_无数据" text:YZMsg(@"你还没有收到任何预约")];
                }else{
                    [PublicView showImgNoData:_collectionView name:@"follow_无数据" text:YZMsg(@"你还没有预约任何人")];
                }
            }else{
    ...
    if (leftBtn && leftBtn.selected == YES) {
        ...
        [YBToolClass postNetworkWithUrl:@"Live.ToAppointment" andParameter:@{@"subscribeid":model.subscribeid,@"sign":sign} success:^(int code, id  _Nonnull info, NSString * _Nonnull msg) {
            if (code == 0) {
                ...
                WeakSelf;
                [[YBImManager shareInstance]sendV2CustomMsg:custom_elem andReceiver:model.userID complete:^(BOOL isSuccess, V2TIMMessage *sendMsg, NSString *desc) {
                    if(isSuccess){
                        NSLog(@"SendMsg Succ");
